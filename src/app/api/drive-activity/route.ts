...
export async function GET() {
  console.log('Received request to create Google Drive listener');
  ...
  if (!clerkResponse || !clerkResponse[0]?.token) {
    return NextResponse.json({ message: 'Google access token not found' }, { status: 500 });
  }
  ...
  console.log('🔥 Webhook URL:', `${process.env.NEXT_PUBLIC_URL}/api/drive-activity/notification`)
  ...
  try {
    ...
    return new NextResponse('Failed to create listener', { status: 500 })
  }
